...
def test_avg_fits_ccdproc(tmp_path):
    ...
    # test median mode
    print("\n**** testing median mode ****\n")
    avg_fits_ccdproc(mode="0", outfile=f"{tmp_path}/averaged/avg_fits_median.fits", fnames=files, verbose=True)
    ...
    # test mean mode
    print("\n**** testing mean mode ****\n")
    avg_fits_ccdproc(mode="1", outfile=f"{tmp_path}/averaged/avg_fits_mean.fits", fnames=files, verbose=True)
    ...
    avg_fits_ccdproc(mode="1", outfile=f"{tmp_path}/averaged/avg_fits_mean.fits", fnames=files, datatype=np.float64, verbose=True)
    mean_data = fits.getdata(f"{tmp_path}/averaged/avg_fits_mean.fits")
