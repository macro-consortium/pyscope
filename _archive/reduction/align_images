...
def get_args():
	...
	parser.add_option('-r', dest = 'reference', metavar='Reference', action = 'store', help = 'Reference FITS image ') 
...
def align_and_crop(im1, im2):
	...
	if dist_ra > new_image.data.shape[0] or dist_dec > new_image.data.shape[1]:
		print('The images are separated by too much distance and cannot be aligned. Exiting...')
		sys.exit(1)
