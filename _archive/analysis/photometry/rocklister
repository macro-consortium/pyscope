...
def get_args():
	parser = OptionParser(usage = "Uasge: %prog [options] FITSimage", description='Program %prog queries MPC online database for Asteroids and Comets',version = vers)
	parser.add_option('-R', dest = 'RA', metavar='RA', action = 'store', help = 'RA to search [hh:mm:ss] (optional, default use FITS header)') 
	parser.add_option('-D', dest = 'DEC', metavar='DEC', action = 'store', help = 'DEC to search [dd:mm] (optional, default use FITS header)')
	parser.add_option('-r', dest = 'radius', default = 15, metavar='Radius', action = 'store', help='Search radius [arcmin], default 15')
	parser.add_option('-m', dest = 'limmag', default = 20, metavar='LimMag', action = 'store', help = 'Limiting magnitude, default 20')
	parser.add_option('-d', dest = 'date',  metavar='date', action = 'store', help = 'Date string [yyyy/mm/dd] (optional, default use FITS header)')
	parser.add_option('-t', dest = 'ut', metavar='time', action = 'store', default = '00:00', help = 'UT time [hh:mm] (optional, default use FITS header)')	
...
def find_mpc_objects_cl(date, ut, ra, dec, radius, limmag):
	...
	ut_hr, ut_min = [float(x) for x in ut.split(':')]; frac_day = (ut_hr + ut_min/60.) /24.
...
if len(args) == 1:
	...
	if os.path.isfile(ftsfile):
		Objects, Ctr_Coords, Obj_Coords, Magnitudes, Offsets, Rates, Comments = find_mpc_objects(ftsfile, radius, limmag)
	...
	if not opts.RA or not opts.DEC or not opts.date: sys.exit( "Must specifiy at least -R, -D, -d if no image specified.")
	ra = opts.RA  ; dec = opts.DEC ; ut = opts.ut ; date = opts.date
	Objects, Ctr_Coords, Obj_Coords, Magnitudes, Offsets, Rates, Comments = find_mpc_objects_cl(date, ut, ra, dec, radius, limmag)	
	ctr_coords = Ctr_Coords.to_string(style ='hmsdms', precision=1, sep=':', decimal =False)
# Print results
N = len(Objects)	
if N == 0:
	...
	print()
	print("Found %i  minor bodies with V < %s  within %s\' radius of %s on %s at %s UT" % \
	...
	for j in range(N):
		...
		ra_deg =  Obj_Coords[j].ra.degree; dec_deg = Obj_Coords[j].dec.degree
