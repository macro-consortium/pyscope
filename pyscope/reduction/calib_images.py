...
@click.option('-t', '--camera-type', type=click.Choice(['ccd', 'cmos']), 
    ...
    help='Camera type.')
@click.option('-i', '--image-dir', type=click.Path(exists=True, file_okay=False, resolve_path=True),
                default='./images/', show_default=True, 
                help='''Directory containing images to be calibrated. If passed, then
                    the fnames argument is ignored.''')
@click.option('-c', '--calib-dir', type=click.Path(exists=True, file_okay=False, resolve_path=True),
                default='./images/', show_default=True, required=True,
                help='Location of all calibration files.')
@click.option('-r', '--raw-archive-dir', type=click.Path(exists=True, file_okay=False, resolve_path=True),
                help='''Directory to archive raw images. If none given, no archiving is done, 
                however, the '--in-place' option is not allowed.''')
@click.option('--in-place', is_flag=True, default=False, show_default=True,
...
@click.option('-b', '--bad-columns', default='', show_default=True,
    help='Comma-separated list of bad columns to fix.')
@click.option('-w', '--wcs', is_flag=True, default=False, show_default=True,
                help='''If given, the WCS is solved for each image. If not given, the WCS is
                not solved.''')
@click.option('-z', '--zmag', is_flag=True, default=False, show_default=True,
...
@click.option('-v', '--verbose', is_flag=True, default=False, show_default=True,
                help='Verbose output.')
@click.argument('fnames', nargs=-1, type=click.Path(exists=True, file_okay=True, resolve_path=True))
@click.version_option(version='0.1.0')
@click.help_option('-h', '--help')
def calib_images_cli(camera_type, image_dir, calib_dir, raw_archive_dir, in_place, 
                    astro_scrappy, bad_columns, wcs, zmag, verbose, fnames):
    
    if verbose:
        logger.setLevel(logging.DEBUG)

    if raw_archive_dir is None and in_place:
        raise click.BadParameter('The --in-place option is not allowed if no raw archive directory is given.')
    
    if image_dir is not None:
        logger.info('--image-dir passed, ignoring fnames argument and using all images in directory.')
        fnames = []
    ...
    for fname in fnames:
        ...
        if raw_archive_dir is not None:
            raw_archive_dir = os.path.join(raw_archive_dir, datetime.datetime.now().strftime('%Y-%m-%d'))
        ...
        flat_frame = f'{calib_dir}/master_flat_{filt}_{readout}_{exptime}_{xbin}x{ybin}.fts'
        ...
        ccd_calib(camera_type=camera_type, flat_frame=flat_frame, dark_frame=dark_frame,
            bias_frame=bias_frame, flat_dark_frame=flat_dark_frame, astro_scrappy=astro_scrappy,
            bad_columns=bad_columns, in_place=in_place, fnames=fname)
