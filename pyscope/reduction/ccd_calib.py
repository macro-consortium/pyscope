...
@click.command(epilog='''Check out the documentation at
                ...
                information.''')
@click.option('-t', '--camera-type', type=click.Choice(['ccd', 'cmos']), 
    default='ccd',
    show_choices=True, show_default=True,
    help='Camera type.')
@click.option('-d', '--dark-frame', type=click.Path(exists=True, resolve_path=True),
    help='Path to master dark frame.')
@click.option('-f', '--flat-frame', type=click.Path(exists=True, resolve_path=True),
    help='Path to master flat frame.')
@click.option('-b', '--bias-frame', type=click.Path(exists=True, resolve_path=True),
    help='Path to master bias frame. Ignored if camera type is CMOS.')
@click.option('-a', '--flat-dark-frame', type=click.Path(exists=True, resolve_path=True),
    help='Path to master flat-dark frame. Ignored if camera type is CCD.')
@click.option('-s', '--astro-scrappy', nargs=2, default=(1, 3), 
    show_default=True,
    help='Number of hot pixel removal iterations and estimated camera read noise.')
@click.option('-c', '--bad-columns', default='', show_default=True,
    help='Comma-separated list of bad columns to fix.')
@click.option('-i', '--in-place', is_flag=True, default=False, show_default=True,
...
def ccd_calib_cli(camera_type, dark_frame, flat_frame, bias_frame, flat_dark_frame, 
                    astro_scrappy,
                    bad_columns, in_place, verbose, fnames):
    if verbose:
        logger.setLevel(logging.DEBUG)
    
    logger.debug(f'''ccd_calib(camera_type={camera_type}, dark_frame={dark_frame}, 
    ...
    for fname in fnames:
        ...
        if image_readout_mode != dark_readout_mode:
            logger.warning(f'Image readout mode ({image_readout_mode}) does not match dark readout mode ({dark_readout_mode})')
        
        if image_readout_mode != flat_readout_mode:
            logger.warning(f'Image readout mode ({image_readout_mode}) does not match flat readout mode ({flat_readout_mode})')
        
        if image_xbin != dark_xbin:
            logger.warning(f'Image X binning ({image_xbin}) does not match dark X binning ({dark_xbin})')
        
        if image_xbin != flat_xbin:
            logger.warning(f'Image X binning ({image_xbin}) does not match flat X binning ({flat_xbin})')
        
        if image_ybin != dark_ybin:
            logger.warning(f'Image Y binning ({image_ybin}) does not match dark Y binning ({dark_ybin})')
        
        if image_ybin != flat_ybin:
            logger.warning(f'Image Y binning ({image_ybin}) does not match flat Y binning ({flat_ybin})')

        if image_filter != flat_filter:
            logger.warning(f'Image filter ({image_filter}) does not match flat filter ({flat_filter})')

        if camera_type == 'cmos':
            if image_exptime != dark_exptime:
                logger.warning(f'''Image exposure time ({image_exptime}) does not match dark exposure time ({dark_exptime}),
                    recommended for a CMOS camera''')
            if flat_exptime != flat_dark_exptime:
                logger.warning(f'''Flat exposure time ({flat_exptime}) does not match flat-dark exposure time ({flat_dark_exptime}),
                    recommended for a CMOS camera''')
        ...
        if camera_type == 'ccd':
            ...
            if dark_frame is not None:
                logger.info('''Applying the dark frame. CCD selected so a bias-subtracted
            ...
            if flat_dark_frame is not None:
                logger.info('''Applying the flat-dark frame. CMOS selected so a dark
        ...
        if flat_frame is not None:
            ...
            if camera_type == 'ccd':
                logger.debug('''CCD selected so the flat frame is
                ...
                logger.debug('''CMOS selected so the flat frame is simply
                        dark-flat subtracted if present.''')
                if flat_dark_frame is not None:
                    flat -= flat_dark
            
            logger.info('''Normalizing the flat frame by the mean of the
        ...
        if np.min(cal_image) < 0:
            logger.info('Minimum value is %i, adding pedestal of equal value' % np.min(cal_image))
        ...
        if astro_scrappy[0] > 0:
            ...
            t0 = time.time()
            mask, cal_image = astroscrappy.detect_cosmics(cal_image, niter=astro_scrappy[0],
                readnoise=astro_scrappy[1])
            t = time.time() - t0
            hdr.add_comment(f'Removed hot pixels using astroscrappy, {astro_scrappy[0]} iterations')
        ...
        if len(bad_columns) > 0:
            ...
            for badcol in bad_columns:
                cal_image[:, badcol] = (cal_image[:, badcol-1] + cal_image[:, badcol+1])/2
        ...
        if in_place:
            ...
            fits.writeto(fname.split('.')[:-1]+'_cal.fts', cal_image, hdr, overwrite=True)
