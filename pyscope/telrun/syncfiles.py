...
@click.option("-p", "--port", type=int, help="SSH port of remote server")
@click.option(
    "-k",
    "--key",
    type=click.Path(exists=True, resolve_path=True),
...
def syncfiles_cli(
    ...
    logger.debug(
        f"syncfiles({username}, {host}, {port}, {key}, {config}, {schedules}, {images}, {logs})"
    )
    ...
    while True:
        if os.path.getmtime(local_config_dir + "/syncfiles.cfg") > syncfiles_mtime:
            ...
            cfg_ssh, cfg_scp = _get_client(uname, hostname, p, k)
            _sync_directory(
                cfg_scp, local_config_dir, remote_config_dir, "send", ".cfg"
            )
...
def _read_syncfiles_cfg(syncfiles_cfg):
    ...
    remote_config_dir = cfg.get(
        "DEFAULT", "remote_config_dir", fallback="/home/telrun/config/"
    )
    local_schedules_dir = cfg.get(
        "DEFAULT", "local_schedules_dir", fallback="./schedules/"
    )
    remote_schedules_dir = cfg.get(
        "DEFAULT", "remote_schedules_dir", fallback="/home/telrun/schedules/"
    )
    local_images_dir = cfg.get("DEFAULT", "local_images_dir", fallback="./images/")
    remote_images_dir = cfg.get(
        "DEFAULT", "remote_images_dir", fallback="/home/telrun/images/"
    )
    local_logs_dir = cfg.get("DEFAULT", "local_logs_dir", fallback="./logs/")
    remote_logs_dir = cfg.get(
        "DEFAULT", "remote_logs_dir", fallback="/home/telrun/logs/"
    )
