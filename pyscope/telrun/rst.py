...
def rst_cli(source=None, date=None, observatory="./config/observatory.cfg", verbose=0):
    ...
    if type(observatory) is str:
        ...
        if os.path.exists(observatory):
            ...
            raise click.BadParameter("Observatory configuration file does not exist: %s" % observatory)
    ...
    t0 = astrotime.Time(datetime.datetime(date.year, date.month, date.day, 12, 0, 0, tzinfo=tz),
        format='datetime')
    ...
    civil_twilight = (
       ('Civil Twilight Start', observer.sun_set_time(t0, which="nearest", horizon=-6 * u.deg)),
        ('Civil Twilight End', observer.sun_rise_time(t0, which="next", horizon=-6 * u.deg)),
    )
    nautical_twilight = (
        ('Nautical Twilight Start', observer.sun_set_time(t0, which="nearest", horizon=-12 * u.deg)),
        ('Nautical Twilight End', observer.sun_rise_time(t0, which="next", horizon=-12 * u.deg)),
    )
    astronomical_twilight = (
        ('Astronomical Twilight Start', observer.sun_set_time(t0, which="nearest", horizon=-18 * u.deg)),
        ('Astronomical Twilight End', observer.sun_rise_time(t0, which="next", horizon=-18 * u.deg)),
    )

    times = [horizon[0], horizon[1], civil_twilight[0], civil_twilight[1],
                nautical_twilight[0], nautical_twilight[1], astronomical_twilight[0],
    ...
    if source is not None:
        ...
        rise_time = ('%s Rise' % source, observer.target_rise_time(t0, obj, which="next"))
        transit_time = ('%s Transit' % source, observer.target_meridian_transit_time(t0, obj, which="next"))
        ...
        if isinstance(rise_time[1].value, np.ma.MaskedArray) and isinstance(set_time[1].value, np.ma.MaskedArray):
    ...
    for t in times:
        tbl.add_row([
            ...
            t[1].strftime("%H:%M"),
            observer.local_sidereal_time(t[1]).to_string(precision=0, sep=":", fields=2)])

    if verbose > -1:
        click.echo(f"Times from: {date.strftime('%Y-%m-%d %H:%M (%Z)')}, {t0.strftime('%Y-%m-%d %H:%M (UTC)')}")
        click.echo(tbl)
