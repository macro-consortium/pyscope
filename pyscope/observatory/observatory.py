...
from ..utils import _args_to_config, _kwargs_to_config, _get_image_source_catalog, airmass
...
class Observatory:
    ...
    def __init__(self, config_path=None, **kwargs):
        ...
        if config_path is not None:
            ...
            if self.camera_driver.lower() in ('maxim', 'maximdl'):
                ...
                self._camera = _import_driver('Camera', 
            ...
            self._cover_calibrator = _import_driver('CoverCalibrator', driver_name=self.cover_calibrator_driver, ascom=self.cover_calibrator_ascom, 
            ...
            self._dome = _import_driver('Dome', driver_name=self.dome_driver, ascom=self.dome_ascom, 
            ...
            if self.filter_wheel_driver.lower() in ('maxim', 'maximdl'):
                ...
                self._filter_wheel = _import_driver('FilterWheel', driver_name=self.filter_wheel_driver, ascom=self.filter_wheel_ascom,
            ...
            self._focuser = _import_driver('Focuser', driver_name=self.focuser_driver, ascom=self.focuser_ascom, 
            ...
            self._observing_conditions = _import_driver('ObservingConditions', driver_name=self.observing_conditions_driver, 
            ...
            self._rotator = _import_driver('Rotator', driver_name=self.rotator_driver, ascom=self.rotator_ascom, 
            ...
            for val in self._config['safety_monitor'].values():
                try: 
                    ...
                    self._safety_monitor.append(_import_driver('SafetyMonitor', driver_name=driver, ascom=ascom, 
            ...
            for val in self._config['switch'].values():
                try: 
                    ...
                    self._switch_kwargs.append(dict((k, literal_eval(v)) for k, v in (pair.split(':')
                        for pair in kw.split())) if kw not in (None, '') else None)
                    self._switch.append(_import_driver('Switch', driver_name=driver, ascom=ascom, 
            ...
            self._telescope = _import_driver('Telescope', driver_name=self.telescope_driver, ascom=self.telescope_ascom, 
            ...
            self._autofocus = _import_driver('Autofocus', driver_name=self.autofocus_driver, 
            ...
            for val in self._config['WCS'].values():
                try:
                    ...
                    self._wcs.append(_import_driver('WCS', driver_name=val,
        ...
        if self._dome is not None: _check_class_inheritance(type(self._dome), 'Dome')
        ...
        if self._filter_wheel is not None: _check_class_inheritance(type(self._filter_wheel), 'FilterWheel')
        ...
        if self._focuser is not None: _check_class_inheritance(type(self._focuser), 'Focuser')
        ...
        if self._observing_conditions is not None: _check_class_inheritance(type(self._observing_conditions), 'ObservingConditions')
        ...
        if self._rotator is not None: _check_class_inheritance(type(self._rotator), 'Rotator')
        ...
        if type(kwarg) not in (iter, list, tuple): 
            ...
            if self._safety_monitor is not None: _check_class_inheritance(type(self._safety_monitor), 'SafetyMonitor')
            ...
            for i, safety_monitor in enumerate(self._safety_monitor):
                if safety_monitor is not None: _check_class_inheritance(type(safety_monitor), 'SafetyMonitor')
        ...
        if type(kwarg) not in (iter, list, tuple): 
            self._switch = kwarg
            if self._switch is not None: _check_class_inheritance(type(self._switch), 'Switch')
        ...
        if self._autofocus is not None: _check_class_inheritance(type(self._autofocus), 'Autofocus')
    ...
    def get_object_altaz(self, obj=None, ra=None, dec=None, unit=('hr', 'deg'), frame='icrs', t=None):
        ...
        logger.debug(f'Observatory.get_object_altaz({obj}, {ra}, {dec}, {unit}, {frame}, {t}) called')
    ...
    def get_object_slew(self, obj=None, ra=None, dec=None, unit=('hr', 'deg'), frame='icrs', t=None):
        ...
        logger.debug(f'Observatory.get_object_slew({obj}, {ra}, {dec}, {unit}, {frame}, {t}) called')
    ...
    def save_last_image(self, filename, frametyp=None, do_wcs=False, do_fwhm=False, overwrite=False, 
        ...
        logger.debug(f'Observatory.save_last_image({filename}, {frametyp}, {do_wcs}, {do_fwhm}, {overwrite}, {custom_header}, {kwargs}) called')
    ...
    def set_filter_offset_focuser(self, filter_index=None, filter_name=None):
        ...
        logger.debug(f'Observatory.set_filter_offset_focuser({filter_index}, {filter_name}) called')
    ...
    def slew_to_coordinates(self, obj=None, ra=None, dec=None, unit=('hr', 'deg'), frame='icrs', 
        ...
        logger.debug(f'Observatory.slew_to_coordinates({obj}, {ra}, {dec}, {unit}, {frame}, {control_dome}, {control_rotator}, {home_first}, {wait_for_slew}, {track}) called')
    ...
    def _parse_obj_ra_dec(self, obj=None, ra=None, dec=None, unit=('hour', 'deg'), frame='icrs', t=None):
        ...
        logger.debug(f'''Observatory._parse_obj_ra_dec({obj}, {ra}, {dec}, {unit}, {frame}, {t}) called''')
    ...
    def telescope_info(self):
        ...
        info['AIRMASS'][0] = _airmass(obj.transform_to(coord.AltAz(obstime=self.observatory_time, location=self.observatory_location)).alt.to(u.rad))
...
def _check_class_inheritance(device_class, device):
    ...
    if not getattr(observatory, device) in device_class.__bases__:
            raise ObservatoryException('Driver %s does not inherit from the required _abstract classes' % device_class)
    else: logger.debug('Driver %s inherits from the required _abstract classes' % device_class)