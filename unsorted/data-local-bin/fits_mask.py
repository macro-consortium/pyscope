...
# Mask a fits image based on a second binary image of the same size
...
if len(sys.argv) == 1:
  print " "
  print "Usage: fits_mask.py infile.fits maskfile.fits outfile.fits"
  print " "
  sys.exit("Mask out pixels in a fits file\n")
elif len(sys.argv) == 4:
  infile = sys.argv[1]
  maskfile = sys.argv[2]
  outfile = sys.argv[3]
...
# Create the output image by masking the input image with the input mask


maskedinimage = ma.array(data = inimage, mask = maskimage, copy = True)
outimage = ma.filled( maskedinimage, 0.0 )
