...
# Create a mean or average image from a stack of fits images
...
if len(sys.argv) == 1:
  print " "
  print "Usage: fits_mean.py  outfile.fits infile1.fits infile2.fits infile3.fits ...  "
  print " "
  sys.exit("Create a mean image from a stack of fits images\n")
elif len(sys.argv) >=4:
  outfile = sys.argv[1]
  infiles = sys.argv[2:]
else:
...
# Use numpy to calculate the mean of the stack as a new image

outimage = np.mean(inimages,axis=0)
...
outhdr['history'] = 'Mean of %d images by fits_mean' %(nimages,)
