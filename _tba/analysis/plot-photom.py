...
def get_args():
	...
	parser = OptionParser(description='%prog plots light curves using output file from program photom', version = vers)
	parser.add_option('-a', dest = 'plottype', metavar='plottype', action = 'store', default= 'png', help = 'Plot type [default png]')
	parser.add_option('-b', dest = 'barycenter', metavar='use barycenter time', action = 'store_true', default = False,help = 'Use barycenter time (requires FITS images) [def. False]')
	parser.add_option('-c', dest = 'check', metavar='show checkstar'  , action = 'store_true', default = False,help = 'Show check star False]')
	parser.add_option('-d', dest = 'double', metavar='show double'  , action = 'store_true', default = False,help = 'Show double phase (0.0-2.0) [default False]')
	parser.add_option('-P', dest = 'period', metavar='period', action = 'store', type = float, default= 1, help = 'Period (days)') 
	parser.add_option('-p', dest = 'plot_phase', metavar='plot_phase', action = 'store_true', default = False, help = 'Plot phase [default off]') 
	parser.add_option('-j', dest = 'jdrange', metavar='jdmin, jdmax'  , action = 'store', default = '0,0',help = 'JD range e.g. 2456722.73,2456724.56 [default all]')
	parser.add_option('-l', dest = 'line', metavar='line', action = 'store_true', default = False, help = 'Draw median line for check star')
	parser.add_option('-J', dest = 'JD0', metavar='JD0'  , action = 'store', type=float, default = 5, help = 'Reference Barycentric Julian date, phase =0')
	parser.add_option('-m', dest = 'mag_ref', metavar = 'Ref. mag.', type=float, default = 0.0, help='Reference star magnitude [default =0]')
	parser.add_option('-r', dest = 'refmag', metavar='refmag'  , action = 'store', type=float, default = 1, help = 'Exclude images whose ref. star diff. mag exceeds refmag')
	parser.add_option('-t', dest = 'tmin', metavar='Time_minimum', action = 'store_true', default = False, help = 'Solve for time of minimum [default False]') 
	parser.add_option('-w', dest = 'width', metavar='width', action = 'store', type = int, default= 20, help = 'Minimum fit width, sample times [default 20]') 
	parser.add_option('-T', dest = 'title', metavar='title'  , action = 'store', default = '',help = 'Alternate plot suptitle [default object]')
	parser.add_option('-v', dest = 'verbose', metavar='verbose'  , action = 'store_true', default = False,help = 'Verbose output [default False]')
	parser.add_option('-y', dest = 'yminmax', metavar='ymin,ymax'  , action = 'store', default = '1,-1',help = 'y axis min, max [default -1,1]')
	
	return parser.parse_args()

def plot_lc_jd(bjd, ymin, ymax, jdmin, jdmax, obj, obj_sigma, ck, ck_sigma, pltname, plottype, suptitle, jd0, show_check, solve_tmin, tmin_args, box_text, line):		
	...
	if jdmin != 0:
		jd_offset = jdmin; xmax = jdmax - jdmin
	else:
		jd_offset = bjd[0]; xmax = bjd[-1] - bjd[0]
	plt.xlim(0,xmax)
	plt.errorbar(bjd-jd_offset, obj, yerr=obj_sigma, ls='none', marker='o',markersize=3)
	if show_check: plt.errorbar(bjd-jd_offset, ck, yerr=ck_sigma, ls ='none', marker='.')
	if use_barycenter:
	...
	if use_barycenter:
		...
		ax.text(0.75, 0.15, box_text, transform=ax.transAxes, fontsize=8, verticalalignment='top', bbox=props)
...
def plot_lc_phs(phs, ymin, ymax, jdmin, jdmax, obj, obj_sigma, ck, ck_sigma, pltname, plottype, suptitle, jd0, show_check, show_double, solve_tmin, tmin_args, box_text):
	plt.figure()
	fig, ax = plt.subplots(1)
	if solve_tmin:
		bjd_min, sigma, phs_min, jd1, oc, xmod, phs_mod, ymod = tmin_args
		bjd_min_frac = bjd_min - jd1
		title = 'BJD0: %.5f, P: %.9f,\n BJDmin: %.5f +/- %.5f,  O-C: %.5f days [%.1f s +/- %.1f s]' % (jd0, P, bjd_min, sigma, oc,oc*86400,sigma*86400)
	else:
		title = 'BJD0: %.5f, P: %.9f day (%.3f hr)' % (jd0, P, P*24.)
	xmin = 0; xmax = 1.0
	if show_double:
		xmax = 2.0
	...
	if show_check: plt.errorbar(phs,ck,yerr=ck_sigma,color = 'g', ls ='none',marker='.')
	if solve_tmin: 
		plt.axvline(phs_min, color='r', lw=1.5, linestyle='dashed')
		plt.plot(phs_mod,ymod,'r.')
	plt.grid(True)
	plt.xlabel('Phase') ; plt.ylabel('Differential magnitude')
...
def calc_bjd(jd_utc, ra_str,dec_str):
	...
	lowell = coord.EarthLocation.of_site('Multiple Mirror Telescope') # close enough to Winer
...
def get_hdr_info(fts_image):
	...
	ra_str = hdr['RA']; dec_str = hdr['DEC']
	object = hdr['OBJECT'].replace(' ',''); filter= hdr['FILTER']; telescope =hdr['TELESCOP']
	exptime = hdr['EXPTIME']; date_obs = hdr['DATE-OBS'][0:10].replace('-','_')
	jd_utc =  hdr['JD'] + exptime/(2.*86400)
...
# Read data
lines = fn.readlines()
BJD = []; ut_hr = []; obj= []; obj_sigma = []; ck = []; ck_sigma = []; phs = []
n = 0
for line in lines:
	...
	if use_barycenter:
		...
		if n ==0:
			objname, ra_str, dec_str, exptime, filter, telescope, date, jd_utc, bjd = get_hdr_info(ftsname)
		else:
		...
		objname = fname.split('.')[0] ; telescope = 'Gemini' ; filter = ''; exptime = 0; ra_str =''; dec_str =''
	
	if verbose: print(ftsname, jd_utc, bjd,a1)
	jd_ok = (jdmin== 0 and jdmax == 0) or jdmin <= bjd <= jdmax
	v1_ok =  -8 < a1 < 8 ; v1_sigma_ok = a2 < 1.0; vref_ok = a5 < refmag
	if v1_ok and v1_sigma_ok and jd_ok and vref_ok:
		hr = math.modf(bjd+0.5)[0]*24
		ut_hr.append(hr); BJD.append(bjd)
		a1 += ref_mag; a3 += ref_mag
		obj.append(a1); obj_sigma.append(a2); ck.append(a3); ck_sigma.append(a4)
		phs.append( ((bjd-jd0) % P)/P)
		n += 1

if verbose: print('%i points read, computing plot...' % n)

# Time sort using BJD
vals = [ [BJD[i], ut_hr[i], obj[i], obj_sigma[i], ck[i], ck_sigma[i], phs[i] ] for i in range(len(BJD)) ]
...
ut_hr = np.array(ut_hr); BJD = np.array(BJD)
obj = np.array(obj); obj_sigma = np.array(obj_sigma)
ck  = np.array(ck);   ck_sigma = np.array(ck_sigma)
...
if solve_tmin:	
	...
	jmin = np.argmax(obj)
	BJD_min0 = BJD[jmin]
	if verbose: print('Found sample minimum at BJD = %.5f (jmin = %i, mag = %.2f)' % (BJD_min0,jmin,obj[jmin]))
	w = width/2  # Width of fit in points
	x = BJD[jmin-w:jmin+w] - BJD[0] ; y = obj[jmin-w:jmin+w]; s = obj_sigma[jmin-w:jmin+w]
	...
	if verbose: 
		print('Fitted minimum at BJD: %.5f +/- %.5f, O-C = %.5f days (%.1f sec +/- %.1f sec). Phase at minimum = %.3f)' % \
		(bjd_min, sigma, oc, oc*86400, sigma*86400, phs_min))
else:
	bjd_min = 0; phs_min = 0; oc = 0; jd1 = 0; sigma =0; xmod =0; phs_mod =0 ; ymod =0
	
# Plot differential magnitude of target, check star vs heliocentric jd
pltname = '%s_%s' % (objname,date)
if suptitle == '': suptitle = objname
Ymin = np.mean(obj) +ymin; Ymax = np.mean(obj) + ymax
tmin_args = (bjd_min, sigma, phs_min, jd1, oc, xmod, phs_mod, ymod)
obj_args  = (objname, ra_str, dec_str, exptime, filter, telescope, date)
box_text = '%s\nDate: %s\nFilter: %s\nExp time:  %.1f sec' % (telescope,date,filter,exptime)
plot_lc_jd(BJD, Ymin, Ymax, jdmin, jdmax, obj, obj_sigma, ck, ck_sigma, pltname, plottype, suptitle, jd0, show_check, solve_tmin, tmin_args, box_text, line)

# If requested, also plot vs. phase using user-supplied ephemeris
if plot_phase:
	plot_lc_phs(phs, ymin, ymax, jdmin, jdmax, obj, obj_sigma, ck, ck_sigma, pltname, plottype, suptitle, jd0, show_check, show_double, solve_tmin, tmin_args, box_text)
