...
def get_args():
	global parser
	parser = OptionParser(description='Program %prog plots 2-color (Sloan g,r) absolute photometric magnitudes and color index g-r using sextractor, ZP mag',version = '%s' % vers)
	...
	parser.add_option('-f', dest = 'fwhm_range', metavar='fwhm_range', action = 'store', default = '1.4,5', help = 'FWHM max (pixels)   [default 1.4,5]')
	...
	parser.add_option('-J', dest = 'jdrange', metavar='jdrange', action = 'store', default = '0,0', help='JD range (JDmin, JDmax)')
...
def get_sexinfo(sexname, exptime, scale):
	...
	n2 = len(V)
	if verbose: print 'Warning, skipped %i stars because they exceeded allowed FWHM range (%.1f,%.1f), use option -f to change' % (n1-n2,fwhm_min,fwhm_max)
...
def parse_config(config_file):
	Objects =[]; Filters = []; Ftsfiles_G = []; Ftsfiles_R = []
...
def get_star_info(Ftsfiles, Filter, Cal_vals): 
	JD = []; Date = []; Mag_all = []; Mag_err_all = []; FitsFile_all= []; Nr_all = []
	...
	for ftsfile in Ftsfiles:
		...
		os.system('sex %s -c %s -CATALOG_NAME %s -DETECT_THRESH %.1f -VERBOSE_TYPE QUIET' % (ftsfile, sex_path, sname, detect_threshold) )
	
		# Get position, magnitude info for each listed star in output file
		Nr_sex, Ra_sex, Dec_sex, Snr, Flux, Fluxerr, Fwhm_sex, Mag_sex, Mag_sex_err = get_sexinfo(sname, exptime, scale)
		nobs = len(Ra_sex)
		...
		if zp > 0:
			...
			if verbose: print 'WARNING: Using default zero-point for %s filter: (ZP = %.2f)' % (filter,ZP)
	...
	# Sort by JD
	JD, FitsFile_all, Date, Nr_all, Mag_all, Mag_err_all = (list(x) for x in zip(*sorted(zip(JD, FitsFile_all, Date, Nr_all, Mag_all, Mag_err_all ))))
...
def mk_plot(j, JD0, Date0, object, Ra_hms, Dec_dms, mjd_g, mjd_r, G_mag, G_mag_err, R_mag, R_mag_err, ywidth, line):
	fig = plt.figure(j, figsize=(12,8))
	ax = fig.add_subplot(111)
	ym = np.nanmean(np.append(G_mag,R_mag))
	ymin = ym + ywidth/2.; ymax = ymin - ywidth
	...
	if jdmin != 0.0: plt.xlim(jdmin - JD0, jdmax - JD0)
	plt.legend(loc=2)
	plt.ylim(ymin,ymax)
	if line:
		xmin,xmax = ax.get_xlim(); xline = [xmin,xmax]
		ym_g = np.nanmedian(G_mag); ym_r = np.nanmedian(R_mag)
		std_g = np.nanstd(G_mag)  ; std_r = np.nanstd(R_mag) 
		yline_g = [ym_g, ym_g] ; yline_r = [ym_r, ym_r]
		plt.plot(xline,yline_g,linestyle= 'dashed', color = 'blue')
		plt.plot(xline,yline_r,linestyle= 'dashed', color = 'red')
		txt = 'Median g = %.2f +/- %.2f,  r = %.2f +/- %.2f, g-r = %.2f' % (ym_g, std_g, ym_r, std_r, ym_g - ym_r)
...
Cal_vals =	 { 'N':(21.5,0.20), 'B':(21.0,0.35), 'G':(22.2,0.28), 'V':(20.5,0.20), 'R':(21.8,0.12), 'W':(20.66,0.05) }
...
fwhm_min, fwhm_max = [float(x) for x in opts.fwhm_range.split(',')] # Maximum allowed FWHM (pixels)

# Parse configuration file
Objects, Ftsfiles_G, Ftsfiles_R, Ra_hms, Dec_dms, Ra_deg, Dec_deg, title = parse_config(config_file)
nstar = len(Objects)


# Expand filenames if needed
if '*' in Ftsfiles_G[0] or '?' in Ftsfiles_G[0]:  Ftsfiles_G = glob.glob(Ftsfiles_G[0])
if '*' in Ftsfiles_R[0] or '?' in Ftsfiles_R[0]:  Ftsfiles_R = glob.glob(Ftsfiles_R[0])


Filters = ['G','R']
for Filter in Filters:
	if	 Filter == 'G': 
		JD_G, Date_G, FitsFiles_all_G, Mag_G, Mag_err_G, Medians_G, Stds_G = get_star_info(Ftsfiles_G, Filter, Cal_vals)
	elif Filter == 'R': 
		JD_R, Date_R, FitsFiles_all_R, Mag_R, Mag_err_R, Medians_R, Stds_R = get_star_info(Ftsfiles_R, Filter, Cal_vals)
'''
print 'G'
...
G_mag = np.transpose(Mag_G); G_mag_err = np.transpose(Mag_err_G)
R_mag = np.transpose(Mag_R); R_mag_err = np.transpose(Mag_err_R)

# Correct for color

for k in range(nstar):
	mark = marker.next()
	plt.errorbar(mjd_g, G_mag[k], yerr = G_mag_err[k], marker= mark, markersize = 8, c='b', label = '%s [G]' % Objects[k] )
	plt.errorbar(mjd_r, R_mag[k], yerr = R_mag_err[k], marker= mark, markersize = 8, c='r', label = '%s [R]' % Objects[k] )

plt.title(title)
plt.legend(loc=2)
plt.ylim(18,12)
if jdmin != 0.0: plt.xlim(jdmin - JD[0], jdmax - JD[0])
...
for k in range(nstar):
	mk_plot(k+2, JD0, Date0, Objects[k], Ra_hms[k], Dec_dms[k], mjd_g, mjd_r, G_mag[k], G_mag_err[k], R_mag[k], R_mag_err[k], ywidth, line)
...
if csvfile !=  '':	 
	...
	for j in range(len(JD_G)):
		str1 =	' '.join('%s: %.3f %.3f' % (Objects[k], G_mag[k][j], G_mag_err[k][j]) for k in range(nstar)  ) 
		s = '%10.5f	 %30s	G  %s\n' % (JD_G[j], FitsFiles_all_G[j], str1)
		fn.write(s)
	for j in range(len(JD_R)):
		str1 =	' '.join('%s: %.3f %.3f' % (Objects[k], R_mag[k][j], R_mag_err[k][j]) for k in range(nstar)  ) 
