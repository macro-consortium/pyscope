...
html_name = (
    "%s/observing_summary" % email_dir
)  # Summary date (one created each day with -daynr suffix
email_all = (
    "%s/daily_emails.txt" % email_dir
)  # Email list  of people who always receive daily summary
with open(
    f"{email_dir}/email_key.txt"
) as f:  # don't want app password in the clear on github
    mail_passwd = f.read().strip()


def writeline(s, face, color, n):
    hfile.write(
        '<font face="%s", font color ="%s", font size = %ipx>%s</font><p>'
        % (face, color, n, s)
    )
    return
...
def plot_stats(UT_fwhm, FWHM, UT_zmag, D_zmag, daynr):
    ...
    ax[1].scatter(
        UT_zmag, D_zmag, marker="o", cmap=colormap, norm=normalize, s=30, c=D_zmag
    )
...
for fname in fts_names:
    ...
    if obscode == "foc" and fil == "G" and "ZMAG" in hdr:
        ...
        UT_zmag.append(ut_zmag)
        D_zmag.append(d_zmag)
    ...
    if "FWHMH " in hdr and not grism:
        ...
        if fwhm > 1.4:  # Don't use crazy low fwhm values
            ...
            UT_fwhm.append(ut_fwhm)

# Bail if no images!
if len(Obs_codes) == 0:
    sys.exit("No images for %s, exiting" % date_now())
...
s = "Robert L Mutel Telescope daily report  %s (day %i)" % (date_now(), daynr)
writeline(s, "arial", "blue", 4)
hfile.write(
    """<font face="arial">
...
s = "<p><b>Observing statistics</b>"
writeline(s, "arial", "blue", 2)
s = "Total number of observing projects: %i" % len(unique_codes)
writeline(s, "arial", "black", 2)
s = "Total number of images: %i" % len(Obs_codes)
writeline(s, "arial", "black", 2)
s = "Moon phase range: %.0f%% - %.0f%%" % (min(Moonphase), max(Moonphase))
writeline(s, "arial", "black", 2)
s = "Median zenith seeing: %.1f +/- %.1f arcsec" % (FZ_median, FZ_sigma)
writeline(s, "arial", "black", 2)
Exp = []
sources = []
for code in unique_codes:
    ...
    Exp_sum = int(sum([Exp_sec[i] for i in indices]) / 60.0)
...
s = "Total observing time: %.1f hrs" % (sum(Exp) / 60.0)
writeline(s, "arial", "black", 2)
...
s = "<b>Objects observed by observer code</b>"
writeline(s, "arial", "blue", 2)
for n in range(len(unique_codes)):
    code = unique_codes[n]
    src_list = ", ".join(sources[n])
    s = "<b>%s</b>: %s" % (code, src_list)
    writeline(s, "arial", "black", 2)
