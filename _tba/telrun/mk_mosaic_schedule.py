...
def get_args():
    ...
    parser.add_option(
        ...
        metavar="Coords",
        action="store",
        help="J2000 coords of field center:  hh:mm:ss, dd:mm:ss",
    )
    parser.add_option(
        "-l",
        dest="overlap",
    ...
    parser.add_option(
        ...
        action="store",
        default="3,3",
        help="Mosaic gridsize, default 3,3",
    )
    parser.add_option(
        "-f",
        dest="filters",
    ...
    parser.add_option(
        "-O",
        dest="obscode",
        metavar="Observer code",
        action="store",
        default="frm",
        help="Observer code name e.g. gaa",
    )
    parser.add_option(
        "-o",
        dest="observer",
        metavar="Observer name",
        action="store",
        default="",
        help="Observer name",
    )
    parser.add_option(
        "-u",
        dest="utstime",
        metavar="UTSTART Time",
        action="store",
        default="",
        help="UTSTART time hh:mm:ss",
    )
...
fov_ra, fov_dec = [float(x) / 60.0 for x in opts.fov.split(",")]
overlap = opts.overlap / 60.0
...
ra0 = c.ra.deg
dec0 = c.dec.deg
dra = (fov_ra - overlap) / np.cos(dec0 * deg)
ddec = fov_dec - overlap
ra_list = np.linspace(ra0 - (nra - 1) * dra / 2.0, ra0 + (nra - 1) * dra / 2.0, nra)
dec_list = np.linspace(
    dec0 - (ndec - 1) * ddec / 2.0, dec0 + (ndec - 1) * ddec / 2.0, ndec
)
...
for i in range(nra):
    for j in range(ndec):
        ...
        ra_str, dec_str = c.to_string(
            style="hmsdms", precision=1, sep=":", decimal=False
        ).split()
        obsfile.write(
            "source %s  ra %s  dec %s repeat %i %s/\n"
...
ttot = ((nra) * (ndec) * repeat * tsum) / 3600.0
nimage = repeat * (nra) * (ndec) * len(filters)
print("============================================================")
print("Mosaic of %s" % field_name)
print("%i filters, %.0f minutes per filter sequence" % (len(filters), tsum / 60.0))
