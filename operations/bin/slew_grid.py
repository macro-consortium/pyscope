...
CENTER_J2000_RA_HOURS = "06:00:00.0"  # RA coordinates at the center of the grid, in J2000 hours
CENTER_J2000_DEC_DEGS = "30:00:00.0"  # Dec coordinates at the center of the grid, in J2000 degrees, Jupiter target
RASTER_GRID_RA_COLUMNS = 36          # Number of columns (distinct RA values) in the grid
RASTER_GRID_DEC_ROWS = 18            # Number of rows (distinct Dec values) in the grid
...
SETTLE_TIME_SECONDS = 2             # Pause for this many seconds after slewing to each target (and before taking an image, if requested)
...
def main():
    ...
    for (target_ra_j2000_hours, target_dec_j2000_degs) in targets:
        ...
        if RECORD_COORDINATES:
            ...
            print("%d, %f, %f, %f, %f" % (target_number, target_ra_j2000_hours, target_dec_j2000_degs, target_ra_jnow_hours, target_dec_jnow_degs), file=coords_file)
            coords_file.close()
