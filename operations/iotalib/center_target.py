...
def center_coordinates_on_pixel(target_ra_j2k_hrs=0, # ra coordinates of target in j2k
    ...
    for attempt_number in range(max_attempts):
        ...
        logging.info('Searching for WCS solution...')
        logging.info('---------- WCS OUTPUT ----------')
        wcs_status = talonwcs.talon_wcs(tempfilename, str(slew_ra_j2k_hrs), str(slew_dec_j2k_deg),
                              config_wcs.values.arcsec_per_pixel_unbinned * binning,
                              config_wcs.values.mirrored,
                              console_output,
                              search_radius_degs)
        logging.info('---------- END WCS OUTPUT ----------')
        
        if wcs_status:
            logging.info('WCS solution found!')
        else:
            logging.info('WCS solution not found, continuing...')
            continue
        
        target_radec_x_pixel,target_radec_y_pixel = talonwcs.radec_to_xy(tempfilename, str(target_ra_j2k_hrs),
                                                        str(target_dec_j2k_deg))
        
        ra_at_target_pixel_j2k_hrs,dec_at_target_pixel_j2k_deg = talonwcs.xy_to_radec(tempfilename,
        ...
        if sync_mount:
            ...
            else: slew_ra_j2k_hrs+error_ra_hrs
