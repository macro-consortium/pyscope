...
CENTER_J2000_RA_HOURS = sys.argv[2]  # RA coordinates at the center of the grid, in J2000 hours
CENTER_J2000_DEC_DEGS = sys.argv[3]  # Dec coordinates at the center of the grid, in J2000 degrees
RASTER_GRID_RA_COLUMNS = float(sys.argv[5])          # Number of columns (distinct RA values) in the grid
RASTER_GRID_DEC_ROWS = float(sys.argv[6])           # Number of rows (distinct Dec values) in the grid
RA_GRID_SPACING_ARCSEC = float(sys.argv[7])         # RA spacing between each gridpoint, in arcseconds
DEC_GRID_SPACING_ARCSEC = float(sys.argv[8])       # Dec spacing between each gridpoint, in arcseconds
EQUAL_SKY_ANGLE = config[12]              # If True, apply cos(dec) compensation to RA offsets so that the amount of apparent motion of the sky is the same regardless of declination

SETTLE_TIME_SECONDS = float(config[10])             # Pause for this many seconds after slewing to each target (and before taking an image, if requested)
PROMPT_BEFORE_STARTING_NEXT_TARGET = config[13] # If True, script will wait for user to hit Enter before moving to each new target

TAKE_IMAGE_AT_EACH_GRIDPOINT = config[14] # If True, Maxim DL will be used to take an image at each target
EXPOSURE_LENGTH_SECONDS = float(sys.argv[4])         # Exposure length of each image, in seconds
...
SAVE_DATA_PATH = r"{MyDocuments}\CenterTargetData\{Timestamp}" # Script data (including coordinates.txt and FITS images) will be saved to this location.
RECORD_COORDINATES = config[15]           # If True, write each J2000 coordinate to a file "coordinates.txt" in the data directory
...
def main():
    ...
    for (target_ra_j2000_hours, target_dec_j2000_degs) in targets:
        ...
        if RECORD_COORDINATES:
            ...
            print("%d, %f, %f" % (target_number, target_ra_j2000_hours, target_dec_j2000_degs), file=coords_file)
            coords_file.close()
        ...
        if TAKE_IMAGE_AT_EACH_GRIDPOINT:
            ...
            if SAVE_IMAGES:
                filename = "image_%03d_ra_%f_dec_%f.fits" % (target_number, target_ra_j2000_hours, target_dec_j2000_degs)
...
def make_raster_scan_grid(center_ra_hours, center_dec_degs, num_ra_columns, num_dec_rows, ra_spacing_arcsec, dec_spacing_arcsec, equal_sky_angle=True):
    ...
    for i in range(num_dec_rows):
        ...
        for j in range(num_ra_columns):
            ...
            if equal_sky_angle:
                ra_offset_degs = ra_offset_degs / math.cos(convert.degs_to_rads(target_dec_degs))
            
            target_ra_hours = center_ra_hours + convert.degs_to_hours(ra_offset_degs)
