...
TARGET_RA_J2000_HOURS = "19:50:47.564" # RA coordinates of the target star, in J2000 hours
TARGET_DEC_J2000_DEGS = "08:52:12.050" # Dec coordinates of the target star, in J2000 degrees
TARGET_PIXEL_X_UNBINNED = 1648.683        # Target X pixel position in unbinned image. Center of the top left corner pixel is (1, 1)
TARGET_PIXEL_Y_UNBINNED = 3985.727        # Target Y pixel position in unbinned image. Center of the top left corner pixel is (1, 1)
TOLERANCE_PIXELS = 2                   # Continue refining pointing until target RA/Dec is within this many pixels of target pixel location
MAX_RESLEW_ATTEMPTS = 5                # Give up after trying to refine pointing this many times
INITIAL_OFFSET_DEC_ARCMIN = 0         # Offset for first image, to avoid blooming on bright target

SETTLE_TIME_SECONDS = 3                # Pause for this many seconds after slewing to each target before taking an image
EXPOSURE_LENGTH_SECONDS = 15           # Exposure length of each image, in seconds
BINNING = 1                            # Image binning (higher binning reduces resolution but speeds up image readout time) 

ARCSEC_PER_PIXEL_UNBINNED = 0.632       # Estimated image scale of UNBINNED image, in arcseconds per pixel
