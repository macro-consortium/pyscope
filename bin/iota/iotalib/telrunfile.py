...
class TelrunScan(object):
    ...
    def __init__(self):
        ...
        self.compress = 0   # 0 for no compression, else a scale factor
...
def read_next_sls(file_stream):
    ...
    while lineno <= LASTLINE:
        ...
        num_stripped_chars = len(line) - old_length
        ...
        if lineno == 0:
            ...
            # 17             hcomp: 1
            try:
                sp.compress = _atoi(bp)
            except:
                logger.warn("Invalid compression flag '%s'; skipping to next scan",
                        bp)
                lineno = 0
                continue
