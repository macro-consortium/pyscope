...
def initialize():
    ...
    _config_home_path = get_default_config_path()
...
def get_default_config_path():
    """
    By default, the "config" directory lives under:
      IOTAHOME\config
    and we live under:
      IOTAHOME\iota\iotalib\configreader.py
    So we need to find the directory "..\..\config" relative to
    our current script location
    """

    config_relative_path = "../../config"
    iotalib_dir = os.path.dirname(os.path.abspath(__file__))
    config_dir = os.path.abspath(os.path.join(iotalib_dir, config_relative_path))
