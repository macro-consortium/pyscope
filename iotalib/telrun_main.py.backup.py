...
def run_scans(telrun_file):
	...
	if check_roof.checkOpen() == False:
            logging.info("Roof is not open. Stopping...")
            return False
	sun_alt_degs = observatory.get_sun_altitude_degs()
	...
	while True:
        ...
        if config_telrun.values.recenter_if_returns_true(scan):
            ...
            logging.info("Switching to filter %s for pointing adjustment", filter_index)
            observatory.set_filter_and_offset_focuser(filter_index)

            telrun_status.mount_state = "SLEWING"

            center_target.save_images = False
            center_target.check_and_refine = False # Just image and recenter once, assuming it'll do a good enough job
            center_target.binning = config_telrun.values.recenter_exposure_binning
            center_target.exposure_length_seconds = config_telrun.values.recenter_exposure_seconds
            center_target.target_ra_j2000_hours = target_ra_j2000_hours
            center_target.target_dec_j2000_degs = target_dec_j2000_degs
            center_target.initial_offset_dec_arcmin = 0
            center_target.target_pixel_x_unbinned = observatory.camera.get_ccd_width_pixels() / 2.0
            center_target.target_pixel_y_unbinned = observatory.camera.get_ccd_height_pixels() / 2.0
            center_target.search_radius_degs = 0.25
            center_target.sync_mount = config_telrun.values.recenter_using_sync
            
            logging.info("Refining telescope pointing for this scan...")
            if not center_target.center_coordinates_on_pixel():
                logging.info("Recentering failed. Marking scan as failed.")
                set_scan_status(telrun_file, scan, "F")
                continue
        else:
            # Normal blind slew
...
def remove_file_if_needed(filepath):
    if os.path.exists(filepath):
        try:
            ...
            logging.warn("Unable to remove file %s: %s", filepath, ex.message)
